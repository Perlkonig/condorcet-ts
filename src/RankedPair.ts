...
type Pairings = Map<Pair, IResult>;
...
export class RankedPair extends CondorcetBase {
    constructor(c: Set<Candidate>) {
        super(c);
    }

    private calcPairs(d: Map<Candidate, CandidateCount>): Pairings {
        const pairs = new Map() as Pairings;
    ...
    public rank(): Candidate[] {
        ...
        // Sort
        const keys: Array<Pair> = [...pairs.keys()];
        keys.sort((a, b) => {
        ...
        while (locked.length > 0) {
            ...
            const winners: Set<Candidate> = new Set([...locked.map((p) => { return p[0]; })]);
            const losers: Set<Candidate> = new Set([...locked.map((p) => { return p[1]; })]);
            const sources: Candidate[] = [...new Set([...winners].filter(x => !losers.has(x)))];
            if (sources.length !== 1) {
